...
def build_rows(start: str, end: str | None) -> list[VixRow]:
    ...
    vvix_frame = download_close_history("^VVIX", start, end).rename(columns={"close_price": "vvix_close"})

    merged = pd.merge(vix_frame, vvix_frame, on="trade_date", how="outer").sort_values("trade_date")
    rows: list[VixRow] = []
